...
assert.match(acceptance, /prompt\/instruction text retrieved from history remains untrusted/i);
...
const codex = wave.codexCompletionContract.required.join("\n");
assert.match(codex, /Do not vendor, fork or deploy upstream code merely to mark inventory completeness/i);
assert.match(codex, /bounded non-secret corpus and read-only GitHub credentials/i);
assert.match(codex, /prompt-injection and stale-history tests/i);
assert.match(codex, /Leave production, A11oy exposure and autonomous code-change authority HOLD/i);
